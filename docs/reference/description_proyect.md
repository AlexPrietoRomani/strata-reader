...
### 4.2. Capa Abstracta PdfBackend (Fase 13 Etapas B & C)
Para robustecer la portabilidad del motor, se ha refactorizado la capa de decodificación mediante la introducción de traits abstractos en Rust:
- **`PdfBackend` / `PdfDoc` / `PdfPage`:** Interfaces polimórficas que desacoplan el análisis topológico del decodificador concreto.
- **Doble motor en tiempo de compilación (Cargo Features):**
  - `pdfium-backend`: Enlace dinámico de alto rendimiento a PDFium, empaquetado por defecto.
  - `pure-backend`: Motor compilable 100% puro Rust libre de dependencias compartidas C/C++.
- **CLI flag `--pdf-backend`:** Permite conmutar dinámicamente entre motores (`pdfium` | `pure` | `auto`) para máxima versatilidad del despliegue.
